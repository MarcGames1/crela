...
import { servicii, INavItem } from '@/StaticData/navItems'
const Stanga = ({serviciuSelectat="", setlecteazaServiciul=(a:string) =>{}
}) => {
  return (
    <div className="relative flex-1 self-center text-center lg:max-w-[650px] lg:text-left">
      <ul className="lg:ml-12">
        {servicii.map((serviciu: INavItem, idx: number) => {
          return (
            <li
        ...
        text-[#9D9D9D] transition-colors md:pb-0 md:text-2xl md:leading-7 "
                onClick={(e) => setlecteazaServiciul(serviciu.name)}
              >
                {serviciu.name}
              </h3>{' '}
            </li>
