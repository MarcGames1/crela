...
const CePutemRealiza = ({ data, sectionHeader  }:{data:IAccordionData[], sectionHeader:string}) => {
  return (<>
    ...
    <div className="flex flex-wrap my-10 drop-shadow-2xl lg:w-1/2 m-auto gap-2">
      {data.map((a: IAccordionData, i: number) => {
          return (
              ...
              defaultChecked
              >
            <Accordion.Title className="text-2xl font-semibold text-transparent bg-clip-text bg-secondary-gradient">
              {a.title}
            </Accordion.Title>
            <Accordion.Content>
